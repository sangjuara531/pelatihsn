					...
					while ($pecah = $ambil->fetch_assoc()):
						...
						<tr>
							...
							<td>
								...
								$label_class = 'secondary'; // default
							
								if ($status == 'pending') {
									$label_class = 'warning';
								} elseif ($status == 'lunas') {
									$label_class = 'success';
								} elseif ($status == 'sedang kirim pembayaran') {
									$label_class = 'primary';
								}
								?>
								<span class="btn btn-<?= $label_class; ?>"><?= ucfirst($status); ?></span><br>

								<?php if (!empty($pecah['resi_pengiriman'])): ?>
									<small>Resi: <?= $pecah['resi_pengiriman']; ?></small>
								<?php endif; ?>
							</td>
